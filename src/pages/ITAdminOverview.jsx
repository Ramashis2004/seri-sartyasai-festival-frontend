  ...
  const load = async () => {
    try {
      ...
      const [m, nr, sy, to, schoolParts, districtParts, schoolTeachersList, districtTeachersList] = await Promise.all([
        ...
        itListTeachers({ scope: 'school', districtId, frozen: 'true' }).catch(() => []),
        itListTeachers({ scope: 'district', districtId, frozen: 'true' }).catch(() => []),
      ]);
      ...
      const stOnlyTeachers = st.filter((r) => isSchoolTeacherRole(roleOf(r)));
      const dtOnlyGurus = dt.filter((r) => isGuruRole(roleOf(r)));
