...
export const ecListSchoolEvents = async () => {
  const { data } = await API.get("/api/event-coordinator/events/school");
  return data;
};
export const ecListDistrictEvents = async () => {
  const { data } = await API.get("/api/event-coordinator/events/district");
  return data;
};
...
export const ecListParticipants = async ({ scope, eventId }) => {
  ...
  const { data } = await API.get("/api/event-coordinator/participants", { params });
  return data;
};
...
export const ecSubmitMarks = async ({ scope, eventId, items }) => {
  ...
  const { data } = await API.post("/api/event-coordinator/marks", payload);
  return data;
};
...
export const ecGetEvaluationFormat = async ({ scope, eventId }) => {
  const params = { scope, eventId };
  const { data } = await API.get("/api/event-coordinator/evaluation-form", { params });
  return data;
};
